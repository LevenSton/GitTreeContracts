...
import {GitTreeDataTypes as DataTypes} from "./GitTreeDataTypes.sol";
...
library Events {
    ...
    event DerivedCollectioinDeployed(
    ...
    event StateSet(
        address indexed caller,
        DataTypes.GitTreeState indexed prevState,
        DataTypes.GitTreeState indexed newState,
        uint256 timestamp
    );

    event BaseRoyaltySet(
        address indexed caller,
        uint256 indexed prevMaxBaseRoyalty,
        uint256 indexed newMaxBaseRoyalty,
        uint256 timestamp
    );
